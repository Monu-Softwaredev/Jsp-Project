...
<%
    ...
    java.sql.ResultSet rs = ParticipantDAO.ShowRecord(pid);
...
<h1 class="text-center bg-primary pt-2 pb-2 mt-2 rounded-5"> Update Participant Page</h1>

<div class="container bg-info w-75 mypg mb-0">

<form method="post" action="updtparti.jsp">
...
<input type="hidden" class="form-control" name="pid" value=<%=pid %>>
  ...
  <select class="form-select" name="eventname">
    <%
      java.sql.ResultSet rs1 = EventDAO.ShowEventName();
   
        while(rs1.next()){
        	String ename = rs1.getString(1);
        	out.print("<option value='"+ename+"'>"+ename+"</option>");
        }
   %>
  </select>
  </div>
    ...
    <input type="text" class="form-control" name="name" placeholder="Enter Participant Name" value='<%= name %>' required>
  </div>
  
  <div class="mb-3">
    <label class="form-label fw-bold">College Name</label>
    <input type="text" class="form-control" name="collegename" placeholder="Enter College Name Here" value='<%= collegename %>' required>
  </div>
  
  <div class="mb-3">
    <label class="form-label fw-bold">Branch</label>
    <input type="text" class="form-control" name="branch" placeholder="Enter Branch" value='<%= branch %>'>
  </div>
  ...
  <div class="mb-3">
    ...
    <input type="number" class="form-control" name="mobile" placeholder="Enter Mobile No Here" value='<%= mobile %>'>
  </div>
  ...
  <button type="submit" class="btn btn-primary mb-2 fw-bold w-100 fs-4"> Update Participant Record</button>
  </div>
  </form>
